...
def evaluate(
    args, model, eval_examples, eval_data, write_to_pred=False, flowgnn_dataset=None
):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Evaluating"):
        ...
        if flowgnn_dataset is None:
            ...
            if graphs is None:
                logger.info(
                    "skipping batch of %d items, graphs is None for indices: %s",
                    len(index),
                    index,
                )
                continue
...
def main():
    ...
    if args.flowgnn_data:
        ...
        logger.info(
            "FlowGNN dataset: %s\n%s",
            flowgnn_datamodule.train.df,
            flowgnn_datamodule.train.idx2id,
        )
    else:
            ...
            for step, batch in enumerate(bar):
                ...
                if (step + 1) % save_steps == 0 and args.do_eval:
                    ...
                    result = evaluate(
                        ...
                        eval_examples,
                        eval_data,
                        flowgnn_dataset=flowgnn_dataset,
                    )
                    ...
                    if eval_f1 > best_f1:
                        ...
                        not_f1_inc_cnt += 1
                        logger.info(
                            "acc does not increase for %d epochs", not_f1_inc_cnt
                        )
